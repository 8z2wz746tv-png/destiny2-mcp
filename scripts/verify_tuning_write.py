...
**这个脚本量出来的结论（2026-09-22，已写进 ADR-014）**：

1. 调谐**写得进去**：清单内的调谐 → 免费接口 `ErrorCode=1`、回读插槽与六维都对、**不花材料**；
2. 判据是"那颗在不在**这件护甲**允许的清单里"（组件 310），不在里面的回 **1675**；
3. **组件 305 / 角色级 plug set 的"这一位能不能插"对调谐不可信** —— 本脚本会把调谐槽里
   **正装着的那颗**都列进"清单外"，而用 `--to` 指一颗被判"清单外"的调谐照样写成功
   （`--piece 6917530188462629544 --to 891771298`）。所以产品里调谐写入**只看 310**。
