...
def _serialize_build_analysis(analysis) -> dict | None:
    """Serialize BuildAnalysis for aggregate tool output."""
    if analysis is None:
        return None
    return {
...
async def find_build(
    ...
    - 碎片属性会自动从 manifest 读取，引擎会自动计算碎片对属性的影响。
    ...
    Examples:
        find_build(player_name="husky#1234", health_target=200, grenade_target=200)
        find_build(player_name="husky#1234", exotic_name="天穹夜鹰", weapons_target=200)
        find_build(player_name="husky#1234", melee_target=150, fragment_names="保护琢面,黎明琢面")
        find_build(player_name="husky#1234", health_target=200, set_bonus_name="兴旺幸存者", set_bonus_count=4)
        find_build(player_name="husky#1234", super_target=70, grenade_target=70, class_target=70, priority_stat="melee")
    """
    svc = get_ctx(ctx)
        ...
        next_actions = [
            {
                ...
                "arguments": {
                    "character": character,
                    "score": recommendation.results[0].score,
                },
            }
        ]
    else:
...
async def equip_build(
    ...
    注意：5件护甲会串行转移（避免 Bungie API 限流），整个过程约需 5-10 秒。

    Args:
        player_name: Bungie 名称。不填则使用默认玩家。
        character: 穿到哪个角色（hunter/warlock/titan，或中文）。
        score: 要穿的配装分数（来自 find_build 结果中的 score 字段）。
        exotic_name: 金装名称（需与 find_build 时一致，用于定位配装）。
        weapons_target: 武器属性目标（需与 find_build 时一致）。
        health_target: 生命属性目标。
        class_target: 职业技能属性目标。
        grenade_target: 手雷属性目标。
        melee_target: 近战属性目标。
        super_target: 大招属性目标。
        fragment_names: 指定碎片名称（逗号分隔），需与 find_build 时一致。
    """
    svc = get_ctx(ctx)
    player_name = resolve_player_name(svc, player_name)
    if not player_name:
    ...
    return await build_svc.equip_by_score(player_name, request, character, score)
