...
# ── 遗失区域：**专家是常驻列表**，27 个地点（按游戏内的目的地顺序） ──────────
# 用户提供的游戏内截图（「World Lost Sector」专家列表）与 Manifest 逐条对上：
# 有「专家」变体的地点共 **27 个**，游戏那一页就是按目的地分组列出来的（地球两大区域 → 涅索斯
# → 月球 → 幽梦之城 → 木卫二 → 王座世界 → 内欧姆那 → 苍白之心），**不是每日轮换**。
#
# 被排除的 4 个（实测）：
# - `空坦克`（纷争海岸）：只有 传说/大师、没有专家变体（目的地已进金库）；
# - `消息，第一/二/三部分`（苍白之心）：一条难度变体都没有，是别的活动被标成遗失区域类型。
#
# 传说/大师是否有「每日轮换」**还没核对过**：官方接口一个都不给（里程碑没有、组件 204 里也没有），
# 而这张截图只能证明专家是常驻列表。核对方式：看游戏里传说/大师那个入口是常驻全部还是每天只放一个。
LOST_SECTOR_GROUPS: tuple[tuple[str, tuple[str, ...]], ...] = (
    ("欧洲无人区", ("拾荒者巢穴", "挖掘区XII", "采石场", "天空码头IV")),
    ("发射基地", ("溪谷迷宫", "黑色移民号花园2A")),
    ("涅索斯", ("汇流", "裂痕")),
    ("月球", ("K1后勤区", "K1启示区", "K1员工宿舍", "K1通讯区")),
    ("幽梦之城", ("星光大殿", "溺亡之愿海湾", "远日点之栖")),
    ("木卫二", ("地堡E15", "永劫地狱", "隐匿虚空")),
    ("王座世界", ("墓冢", "萃取地", "蜕变")),
    ("内欧姆那", ("惊颤竞速", "水培区", "镀金箴言")),
    ("苍白之心", ("破碎深渊", "繁盛深渊", "遗忘深渊")),
)
LOST_SECTOR_LOCATIONS: tuple[str, ...] = tuple(
    name for _destination, names in LOST_SECTOR_GROUPS for name in names
)
LOST_SECTOR_TOTAL = len(LOST_SECTOR_LOCATIONS)
LOST_SECTOR_ANCHORED = False                 # 传说/大师的每日顺序未核对
LOST_SECTOR_VERIFIED_AT = "2026-09-21"
LOST_SECTOR_VERIFIED_AGAINST = (
    "用户提供的游戏内「World Lost Sector」专家列表截图：27 个地点、按目的地排序（专家=常驻，不是轮换）"
)
