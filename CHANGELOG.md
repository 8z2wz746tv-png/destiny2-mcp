...
**同名多版本不再静默张冠李戴 + 三条真机摩擦修掉**（来自一份外部实测报告，其中一条是报告读错、根因在我）：

- **同名多版本如实报**（报告里的 P0）：`perk_pool`/`god_roll`/`analyze` 这些定义级 intent 按名字
  只能挑一个 `item_hash`，而复刻/重发武器同名不同 hash。真机复现（「千码凝视」）：账号副本
  `1648948519`、按名字解析到 `4164201232` —— 剥掉强化版后缀 `↑` 按栏位比，**枪管 11/7 交集 7 ✓、
  弹匣 7/5 交集 5 ✓，特性栏 6/6 交集 0 ✗**，确实是两个版本。现在身份块带
  `name_variants`（全部同名 hash）+ `name_variants_note`（本次用哪个、要定论就用账号副本的
  `compare` 行视图）。**不新增参数**：定论走 `compare`（组件 310 的可切换项）。
- **行视图不再藏掉固定栏**（这条是报告读错、我背锅）：`compare` 副本行原本"只列有得选的栏"，
  于是"特性栏固定（只有 1 项）"的枪看起来像**没有特性栏**，被读成"半成品"。现在 roll 定义栏
  （框架/枪管/弹匣/特性/起源）**一律照列**，只有 1 项时标 `fixed: true`。
- **`duplicates` 默认 10 → 5 组**（10 组 ≈ 38 KB，在只发标量的宿主上会直接落盘），并在回执里
  给下一步与判据：「这里只给现在装着的 perk，不能用来定级」+「判留哪把看 `compare` 的整栏
  `options`」+「**光等低、没进社区清单都不是弃的理由**」。
...
- 压成 `{name, slot}` 之后，「空模组插槽」(`v400.weapon.mod_empty`)、「3阶：填装速度」
  (`v400.plugs.weapons.masterworks.stat.reload`) 和「维度偏移」(`frames`) 长得一模一样，
  而"留哪把"正是这个 intent 的用途 —— 混在一起会直接污染判断。
- 现在**按插件类别**（不是按名字）拆三块：`perks[]`（框架/枪管/弹匣/特性/起源）、
  `mods[]`（装着的武器模组；空插槽不是"装着的"）、`masterwork`（大师杰作那一颗，没有就不给这个键）。
- 语料那行跟着加断言（`mods` 里不许出现空插槽、`perks` 的 slot 不许是 `mod_empty`/`reload` 这些），
  单测钉住"按类别判、不按名字判"（`3阶：…` 与 `大师杰作：…` 名字不同但都是 masterworks）。
...
- `build_assistant(intent="find"/"recommend")` 新增 `functional_mods`：把社区作者写的部位功能模组
  （抗性/弹药搜寻/回收/吸引这类**流派取向**）传进来，它们会跟着配装一起装。
  社区模板的 `solver_handoff` 里已经带了这份清单，可以原样传。
- **功能模组不进求解器**（用户拍板：这东西算不出来，本来就是照抄）：求解器只是被告知
  "这些能量别动"（按部位预留，取 `max(已装的部位模组, 预留额度)`），然后在剩下的能量里解六维。
- 同名多版本全留着，执行时按"这一位能不能插"挑（其次挑便宜的）；预留能量取最贵那版，
  保证挑中的不超预算。认不出来的如实点名（真机实测：社区模板里「回收利用」在 Manifest 就对不上；
  属性模组传进来会被拒并说明"那半边由六维求解给"）。
- **插不进/装不下 → 跳过 + 点名**（`mod_blocked` + warnings），不拆玩家别的模组、不许整条失败或回退。
- 回执：摘要写"含照抄社区配装的 N 颗功能模组，已从六维求解里扣掉它们的能量"，
  结果里带 `functional_mods`（`mods`/`energy_by_slot`/`unresolved`），确认信封逐件用
  `copied_from_template: true` 标出来源。
- 真机验收（hunter）：3 颗照抄模组 → `find` 28.7 秒出 3 套候选 → `equip_build` 93.5 秒写入，
  `steps` 逐颗写中文名（`模组 '特殊武器弹药搜寻者' → '至高狂徒面具'`、`'震荡阻尼器' → '共振狂怒胸甲'`、
  `'绝缘' → '快速装弹松身裤'`）。**同一份模板的 15 颗全上时，原来能达标的六维目标变成无解**
  （预留 9/8/7/7/7 点 / 上限 11）—— 这是护甲 3.0 的能量规则，回执要如实说清是照抄模组吃掉了能量。
- 结构：新增 `build/functional_mods.py`（解析）、`services/loadout_functional_mods.py`（执行时的现场决策），
  并把"能量腾挪"从 `loadout_mod_sockets.py` 拆去 `services/loadout_energy_budget.py`
  （该模块上限 562 → **518**）；三个新模块都登记了上限。`build_service.py` / `assistants.py` /
  `loadout_equipment_service.py` 因为新入参的透传各 +2～9 行，上限跟着显式上调并写了理由。
- 第一次写入验证（先做的）：`equip_mod` 能写部位功能模组 —— 光泽护臂槽 1 回天掌法 → 鼓舞爆弹
  （能量 10/11 → 9/11，写后回读核对通过），随后原样还原（回天掌法、10/11）。
...
- **同名多件不再报成"失败"**：`move` 只说名字、而账号里有 8 件同名护甲时，以前回 `move_failed`，
  调用方会当成"搬失败了"去重试；现在回 `item_disambiguation_required` —— **没写、也没失败**，
  缺的只是"选哪一件"。候选清单只在信封里发一份（以前信封与 `data.result` 各发一份，读两遍）。
- **确认信封都要说下一步**：所有写入 intent 的 `confirmation_required` 都带
  "得到明确同意后用同一组参数 + `confirmed=true` 重发；在那之前账号不会被改动"，
  `candidates[0].intent` 说明这次要改什么。`loadout_assistant(intent="save")` 不再塞它不读的
  `loadout_id`/`slot_number`，改为一块 `save_preview`：五件护甲（部位中文名 + 已装模组）、武器、
  子职业、件数（只算进配装的三类；真机上 17 件里 8 件是幽灵/载具/飞船）与模组数 ——
  与 `save_loadout` 共用同一段采集，读不到给 `available=false` + 原因，**不阻断确认**。
- **`equip_loadout` 补上回读核对**：以前写完直接说"已装备"，回执里没有一步证明装备真在身上。
  现在与 `equip_build` 同一条纪律（`write_readback` 8×1.5 秒重试）：核对通过 → "已装备，回读核对通过"；
  核对不上只说"回读没确认"（同步窗口），**不改写入结论**；核对自己炸了也不把写成功报成失败。
  真机复验：29.3 秒走完搬运 + 模组 + 子职业 + `verify success=True`。
- **模组/子职业步骤真的写中文名了**：`mod_label` 从 `get_item_info()` 的返回值里读
  `displayProperties`，而那个访问器给的是规范化小字典（键叫 `name`）→ 真机上每次都取空、一路退回
  hash：文档写着"steps 里的模组写中文名"，实际 30 条步骤全是数字（单测没抓到，是因为替身返回的是
  原始定义）。现在只从 `get_item_name()` 取；真机复验 `已经装着 '手雷模组'` / `plug '赌徒闪身'`。
...
**体验：换模组说清代价与核对结论，存配装直接给 ID** —— 2026-09-24（真机走查抓到）：

- `equip_mod` 的确认与回执现在给**六维净变化**（`+10 武器 / −10 手雷`，`from.stat_bonus`
  由服务层给出）——以前只说"加了什么"，用户以为白赚 10 点。
- `equip_mod` 加**写后回读**（与 `equip_build` 同一口径）：读到说"已回读核对"，
  3–10 秒同步窗口内没读到就记 `verified: null` + 原因（**读不到不等于没装上**），
  不再把"自己去看 item"写成必须做的下一步。
- `loadout_assistant(intent="save")` 的回执带 `loadout_id`：以前存完想穿还得再查一次列表
  （而列表一次 121 KB），现在可以直接 `equip_loadout`。

## 0.7.4 — 2026-09-24

**修复：写入失败不再"没有原因"，回滚不再误报「恢复不完整」** —— 2026-09-24（真机走查抓到）：

- `equip_build` 真机失败时回执里 `apply` 步骤的 `detail` 是**空串**——`TimeoutError()` 这类异常的
  `str()` 就是空的，而回执直接写 `str(exc)`。现在统一走 `exceptions.describe_exception()`：
  空消息也给"上游超时（写入是否生效未知，回滚会核对）"或异常类型，并 `logger.exception` 留 traceback。
- 回滚恢复模组那段漏了 1679 判据（`ok = ErrorCode == 1`）：上游回「这个槽已经装着它」被记成失败，
  累积成 `all_ok=False` → 连验证都不跑就报"自动恢复不完整；请检查角色装备"——
  **而账号其实已经完全恢复**（真机读回：原装五件、披风调谐都回到执行前）。
  现在与另两条写入路径共用 `plug_already_installed()`，回执写"已经装着，未改动"。

## 0.7.3 — 2026-09-23

**发版前复验**：全量测试 1800 passed；ruff 干净；护甲语料真机 **26/26 PASS**；
真机 `benchmark_ladder.py` 294.7 秒 → **187.0 秒**；写入细分计时（幂等那次）**25.3 秒**。

**性能：无解阶梯的逐档试解也并发（294.7 → 187.0 秒）** —— 2026-09-23：

- `find_build` 多了两个**只给只读探测**用的关键字参数：`compute=`（走并发档）与
  `register=False`（不把探测出的 200 套塞进候选暂存，那会把调用方要装备的那份挤掉）；
  服务端给出薄入口 `probe_find_build`，阶梯只调它。
- 阶梯改成"档与档并发、档内严格顺序"：结论与串行逐字相同（`ceiling`/`precision`/`trials`
  按原顺序组装）。真机：单次试解因抢 CPU 从 33–35 秒升到 40–47 秒，但 wall 时间赢。
  **阶梯累计 427.2 → 294.7 → 187.0 秒。**

**写入路径细分计时：已经没有"大块肉"了** —— 2026-09-23：

- 幂等的一次 `equip_build`（模组预检发现都装着、一颗没写）实测 **25.3 秒**：
  护甲快照 11.1+2.1、profile 抓取合计 ~15.5、回滚快照 2.5、五件搬运 12.3、批量装备 3.8、回读 3.2。
  要写模组那次仍 ~65 秒（每颗 1–3 秒）。
- **搬运的有界并发实测不可行**：账号写入走 `ReentrantAsyncLock`，它**按 task 可重入** ——
  拆成并发 task 后每个都要抢同一把锁、而持锁的正是等它们的外层任务 → 死锁
  （真机卡死 7 分钟、零写入、账号无改动）。搬运保持顺序，代码抽到
  `services/loadout_transfer_step.py`（为 520 行上限腾位置）。

**新增：`scripts/audit_chain_timing.py`**（只读审计，把装备链路拆成"服务端 vs 间隔"）。
首次结账：27 条历史链路的墙钟 p50 **545.7 秒**、服务端 p50 256.4 秒、**间隔 p50 277.5 秒（51%）**、
**调用 p50 24 次** —— 剩下的时间主要在调用次数上，不在服务端。

## 0.7.2 — 2026-09-23

**发版前复验**：全量测试 1800 passed；ruff 干净；护甲语料真机 **26/26 PASS**；
真机 `benchmark_ladder.py` 改前 427 秒 → 改后 **295 秒**（`analyze` 那一半 227 → 103 秒）。

**性能：无解时的六维阶梯 427 秒 → 295 秒** —— 2026-09-23：

- `analyze` 的六项单项上限（"把点全堆一项能到多少"）互不依赖，以前**串行**跑六次求解器，
  真机实测 **227 秒**；现在走新的只读并发档（`BuildCompute(capacity=4)`）→ **103 秒**。
  主链路仍是串行档（`capacity=1`），写入路径一个字没碰。
- 阶梯第一档"原样"就是调用方刚刚跑过的那次 0 候选解 —— 不再重复求解（`base_order_empty`），
  少一次 ~35 秒的试解。`trials`/`ceiling`/`shortfall` 逐字不变（单测比对两份输出）。

**新增：`scripts/benchmark_ladder.py`**（无解阶梯分段计时，只读）。

## 0.7.1 — 2026-09-23

**发版前复验**：全量测试 1799 passed；ruff 干净；真机 `equip_build` 改前 335 秒（失败 + 回退）
→ 改后 **64.7 秒成功**（同一支脚本 `scripts/benchmark_equip_chain.py`，同日）。

**修复：装备不再因为"已经装着"或"这一位装不上"而整条失败 + 回退** —— 2026-09-23（ADR-018）：

- 1679 `DestinySocketAlreadyHasPlug` 上游是包在 **HTTP 500** 里回的，以前被当失败 →
  整条配装回退（一次 ~215 秒）而账号其实是好的。现在三条写入路径（`equip_build` 模组、
  子职业 plug、`equip_mod`）共用 `plug_already_installed()`：**已经装着 = 状态已成立**。
- 子职业插槽每次都按"当前配置"重写一遍，于是**每次装备都必然撞 1679 并回退** ——
  现在先比对现状，已装的那颗**根本不调用上游**（省掉注定失败的那次写入 + 四次退避重试）。
- 预检遇到"这一位装不上"的模组（组件 207 清单里没有）以前直接抛错 → 整条配装失败 + 回退；
  现在标成 `mod_blocked`，**跳过这一颗、不回退**，摘要与 `warnings` 里如实说清原因与插入条件。

**改进：写后回执能自己说明白，调用方不必再逐件复查** —— 2026-09-23：

- `steps` 里的模组写**中文名**（以前是 hash），`verify` 步骤在"只因模组没装上而对不上"时直说原因；
- 装备链路的最短走法写进技能文档：`find` → 看预览并等确认 → `equip_build(execution_id/canonical_build, confirmed=true)`
  → 看 `steps` 汇报；**不要**再逐件 `intent="item"` 复查（实测一次链路因此多 5 次往返）。

**新增：金装名字「唯一精确匹配」时不再多问一轮** —— 2026-09-23（ADR-017）：

- 以前所有金装请求都先回一句"请确认哪件"，再来一次才求解（真机审计里每次多 20–40 秒的模型回合）；
  现在 `resolve_exotic_armor` 的 `exact` 直接求解，响应用 `query.exotic_resolution` 交代用的是哪件；
  模糊/多件仍然必须停下来确认（一个字没改）。

**新增：`scripts/benchmark_equip_chain.py`**（装备链条分段计时；默认只读，`--write` 才真写账号）。
...
全量语料唯一一条 FAIL（帧表 camelCase 键违反信封规则）已修，复验信封 0 违规。
