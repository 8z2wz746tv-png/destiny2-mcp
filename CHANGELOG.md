...
**新增能力：锻造图样查询** `weapon_assistant(intent="patterns")`（别名 `pattern`/`craft`/`锻造`/`图样`/`图样进度`）：
游戏里「收藏品 → 模式和催化」那一页的 **183 条武器图样**，给每条的进度（就是游戏里那条「图样进度 4/5」）、
还差几次、需求萃取次数、武器类型/槽位/稀有度，以及 Starside「锻造武器来源」的掉落来源。
只读、不改账号；`未开始` = 账号里没有这条图样记录（`progress: null`），**不是 0/5**。
口径与取舍见 ADR-009 与 `docs/plans/PATTERN_QUERY_PLAN.md`。
...
- 图样总数按 **183 条**算（记录名 == 可锻造武器名，精确相等；实测 183/183 命中），
  **不是** `is_craftable` 的 219 件 —— 后者多出 36 件 `（专家）/（失时）/（痛苦）`变体，
  它们不单列图样；单独问变体名会指回基础版。
...
- 新增守门：`tests/test_pattern_query.py`、`tests/test_crafting_sources.py`；语料新增
  `patterns` 行（计数自洽、"未开始 ≠ 0"、变体指回、来源出处）与别名等价行。
