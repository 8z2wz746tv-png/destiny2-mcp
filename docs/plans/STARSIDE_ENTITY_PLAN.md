...
1. 归档里 `derived.*`（`release`/`season`/`foundry`/`craftable`）是站点自己的推导 —— 对外的措辞按
   "Starside 整理"标源可以吗？（我的建议：可以，标注来源与快照时间，并写明"非 Bungie 官方字段"。）
2. 神器关联只到**赛季 28**（当季 29 不在归档里）。要不要在响应里明确"本季神器作者还没整理"？
   （我的建议：要，且这句写在缺口说明里，而不是让玩家以为查不到就是没有。）
3. `dps`/`total_damage`/`frameStats` 这类数值要不要在响应里给"适用条件"（弹药类型/是否含增伤/打什么目标）？
   （我的建议：要 —— 只给数值不给条件会变成误导。）
