...
12. **调谐写不进去**（**2026-09-22 已推翻，见下**）：当时免费插槽接口对调谐回 1663，
    ...
    真机重验（`scripts/verify_tuning_write.py --apply`）的结论是**换得动**，判据是"那颗在不在
    **这件护甲**允许的清单里"（组件 310）：清单里 → `ErrorCode=1`、不花材料；不在 → 1675。
    （**2026-09-22 再更新**：写入路径已开放，`canonical_build` 现在带着这件允许的调谐插件，
    所以"不能进 `canonical_build` 假装可执行"这条**已不成立** —— 它不再需要"假装"了。）
