...
7. 调谐额度不能拿"整个仓库"相加：一套护甲一个部位只出一件，必须每个部位取最强的一件
   （实测 118 件会算出"每项能补 563 点"这种荒唐值）；
8. 规划基线不能把求解器已经配好的模组再加一遍（同一笔模组算两次 → 方案被判成"已达标"而跳过）；
9. 剪枝不能假设"每件最多 +5"：撤掉反向调谐就是 +10，而且模组预算也要算进去，
   否则可行组合会被整支砍掉；
10. 调谐的 −5 打在本就为 0 的项上是**白给**（游戏里属性夹在 0；T5 词条本来只有三项非零）——
    **结论对，理由要补**（2026-09-22 用户拍板 + 实测）：304 会如实报 `生命 −5`，那是**记录值**；
    效果上确实夹在 0，所以见底的项再扣不掉点数。**但"夹 0"只能夹在比较层**
    （`delta_of`、求解器看到的六维），**存储层（`base`/`stats_with`/`Armor.stats`）必须不夹**，
    否则反推 `observed − delta` 会凭空长出点数。口径定案见
    `docs/plans/SOLVER_OPTIMALITY_PLAN.md` 的四·十七。
11. **写入失败会被报成成功**：Bungie 把错误放在 200 响应的信封里（`ErrorCode != 1`），
    而 `apply()` 以前不看内容就 `success: True` —— 实测换调谐账号一个字节没变、工具却说换好了（0.1.8 修）；
12. **调谐写不进去**（**2026-09-22 已推翻，见下**）：当时免费插槽接口对调谐回 1663，
    于是定成"调谐只能作为给玩家的手动清单"。那个 1663 是 ADR-012 查出的字段名 bug 造成的；
    真机重验（`scripts/verify_tuning_write.py --apply`）的结论是**换得动，但只能换成你已经
    拥有的那一颗**：换成身上别的护甲正装着的那颗 → `ErrorCode=1`；换成没有的一颗 → 1675
    `DestinyCannotAffordMaterialRequirements`（要材料）。所以"不能进 `canonical_build`
    假装可执行"这条**仍然成立**（本项目没开写入路径），但理由不是"上游不允许"；
13. 求解器只留排名前 200 套（`RETURNED_ARMOR_SETS`），而放宽那一趟是按**放宽后的目标**
    排名的 —— 可救的方案会排在 200 名外，表现为"明明能补却说补不上"。实测同一组目标
    （武器150+生命103）：池 200 → 救回 0 套，池 1500 → 救回 16 套。修法见 CHANGELOG 0.1.6。
