...
| `weapon_assistant(analyze)`（**未动，仅基线**） | `weapon_name="遗产"` | **68.6 KB**（**只记基线，这轮不动**） | `inventory` 34.2 KB（含 `instances` 30.5 KB）+ `sockets` 21.1 KB + `weapon` 6.5 KB + `starside` 2.9 KB + `god_roll` 2.1 KB + `stats` 1.5 KB；信封 186 B |
