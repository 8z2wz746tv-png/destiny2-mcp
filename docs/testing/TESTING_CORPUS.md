...
- `scripts/run_corpus_pvp_rows.py` —— PvP/生涯**口径**的真机语料（14 行，十几秒跑完）：游戏内 ID、生涯三档（现存/已删/账号级）、计数器 124,495、试炼/铁旗/赛季、`period=season` 如实失败、武器榜 `all_modes`、模式词表外报错、equip 只给计划、**PGCR 参与者名一律游戏内 ID**、**排行榜空响应如实上报**。盯的是**口径**，信封与形状仍归 `run_corpus_all_rows.py`。
