...
- **`find`/`recommend` 默认只给候选行**（0.7.10 起）：每套是 `execution_id` + 分数 + 六维 + 金装/套装 + 这套是哪五件（名字/部位/实例 ID/光等/能量/调谐名）。求解器内部字段与 `canonical_build` 都**不在默认响应里**——要看逐件预览或装备，用 `execution_id` 调 `equip_build`（先 `confirmed=false` 拿预览，同意后再 `confirmed=true`；编号 30 分钟内有效）。`duplicates` 同理：每实例的 perk 只给 `{name, slot}`，要完整 perk 资料走别的入口。
